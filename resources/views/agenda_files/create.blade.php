...
<div class="container mx-auto p-4 max-w-lg">
    ...
    <form action="{{ route('agenda_files.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="file_name" value="{{ old('file_name') }}" class="border px-2 py-1 w-full rounded">
        </div>

        {{-- ファイルタイプ --}}
        <div class="mb-4">
            <label class="block font-medium mb-1">ファイルタイプ</label>
            <input type="text" name="file_type" value="{{ old('file_type') }}" class="border px-2 py-1 w-full rounded">
        </div>

        {{-- 説明 --}}
        <div class="mb-4">
            <label class="block font-medium mb-1">説明</label>
            <input type="text" name="description" value="{{ old('description') }}" class="border px-2 py-1 w-full rounded">
        </div>
