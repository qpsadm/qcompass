    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('admin.certifications.update', $certification->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="level" class="border px-2 py-1 w-full rounded">
                    ...
                    @foreach ($levels as $key => $label)
                        <option value="{{ $key }}" {{ old('level', $certification->level) == $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="checkbox" name="is_show" value="1" {{ old('is_show', $certification->is_show) ? 'checked' : '' }}>
                <span>公開する</span>
            </div>
            ...
            <div class="text-center">
                ...
                <a href="{{ route('admin.certifications.index') }}" class="ml-4 text-gray-600 hover:underline">
                    キャンセル
                </a>
            </div>
        </form>
