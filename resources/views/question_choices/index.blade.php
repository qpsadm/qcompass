...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">選択肢一覧</h1>
        <a href="{{ route('question_choices.create') }}"
            class="bg-blue-500 text-white px-4 py-2 rounded mb-4 inline-block">新規作成</a>

        <table class="table-auto border-collapse border w-full">
            <thead>
                <tr>
                    <th class='border px-4 py-2'>紐づく設問ID</th>
                    <th class='border px-4 py-2'>選択肢本文</th>
                    <th class='border px-4 py-2'>並び順</th>
                    <th class='border px-4 py-2'>正解フラグ</th>
                ...
                @foreach ($question_choices as $QuestionChoice)
                    <tr>
                        ...
                        <td class='border px-4 py-2'>
                            <a href="{{ route('question_choices.show', $QuestionChoice->id) }}"
                                class="text-green-600">詳細</a>
                            <a href="{{ route('question_choices.edit', $QuestionChoice->id) }}"
                                class="text-blue-600 ml-2">編集</a>
                            <form action="{{ route('question_choices.destroy', $QuestionChoice->id) }}" method="POST"
