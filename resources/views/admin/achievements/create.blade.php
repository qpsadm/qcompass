...
@section('content')
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form
                action="{{ isset($Achievement->id) ? route('admin.achievements.update', $Achievement->id) : route('admin.achievements.store') }}"
                method="POST">
                @csrf
                            ...
                            <td class="px-4 py-2">
                                <select name="condition_type" class="border px-3 py-2 rounded w-full">
                                    ...
                                    @foreach ($types as $key => $label)
                                        <option value="{{ $key }}" @selected(old('condition_type', $Achievement->condition_type ?? '') == $key)>{{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="mt-6 flex gap-3">
                    <button type="submit" class="save bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded shadow">
                        {{ isset($Achievement->id) ? '更新' : '保存' }}
                    </button>
                    <a href="{{ route('admin.achievements.index') }}"
                        class="back bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded shadow">
                        一覧に戻る
                    </a>
                </div>
            </form>
