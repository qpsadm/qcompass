    ...
    <div class="container max-w-6xl">
        <div class="bg-white rounded-lg shadow-md p-6 mx-auto">
            <h1 class="text-2xl font-bold mb-6 text-gray-800">
                {{ isset($agenda->id) ? 'アジェンダ 編集' : 'アジェンダ 作成' }}
            </h1>
            ...
            <form id="agenda-form"
                ...
                <table class="w-full table-auto border-collapse">
                    <tbody>
                        {{-- アジェンダ名 --}}
                        <tr class="border-b">
                            <th class="w-60 px-4 py-2 bg-gray-100 text-right font-medium">アジェンダ名
                            ...
                            <td class="px-4 py-2">
                                <input type="text" name="agenda_name"
                                    ...
                                    class="border rounded px-3 py-2 w-96" required>
                            </td>
                        </tr>

                        {{-- カテゴリ --}}
                        <tr class="border-b">
                            <th class="w-60 px-4 py-2 bg-gray-100 text-right font-medium">カテゴリ
                                <span class="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded ml-1">必須</span>
                            </th>
                            <td class="px-4 py-2">
                        ...
                        {{-- 内容・概要 --}}
                        <tr class="border-b">
                            <th class="w-60 px-4 py-2 bg-gray-100 text-right font-medium">内容・概要</th>
                            <td class="px-4 py-2">
                        ...
                        {{-- 表示フラグ --}}
                        <tr class="border-b">
                            <th class="w-60 px-4 py-2 bg-gray-100 text-right font-medium">表示フラグ</th>
                        ...
                        {{-- 承認状態 --}}
                        <tr class="border-b">
                            <th class="w-60 px-4 py-2 bg-gray-100 text-right font-medium">承認状態</th>
                            <td class="px-4 py-2">
                                <select name="status" class="border rounded px-3 py-2 w-60">
                                    <option value="yes"
                                        {{ old('status', $agenda->status ?? '') == 'yes' ? 'selected' : '' }}>承認済み</option>
                                    <option value="draft"
                                        {{ old('status', $agenda->status ?? '') == 'draft' ? 'selected' : '' }}>下書き
                                    </option>
                                </select>
                            </td>
                        </tr>
                ...
                {{-- 新規作成のとき、表示するファイルがないため、以下をコメントアウト　福島 --}}
                {{-- 画像一覧 --}}
