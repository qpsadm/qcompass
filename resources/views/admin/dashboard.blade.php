                    ...
                    <ul class="space-y-2">
                        @foreach ($latestAgendas as $agenda)
                            <li class="border-b pb-2">
                                ...
                                <div class="text-sm text-gray-500">{{ $agenda->created_at->format('Y-m-d H:i') }}</div>
                            </li>
                        @endforeach
                    </ul>
                    ...
                    <ul class="space-y-2">
                        @foreach ($latestAnnouncements as $ann)
                            <li class="border-b pb-2">
                                ...
                                <div class="text-sm text-gray-500">{{ $ann->created_at->format('Y-m-d H:i') }}</div>
                            </li>
                        @endforeach
                    </ul>
                    <div class="mt-3 text-right">
                    ...
                    <ul class="space-y-2">
                        @foreach ($latestReports as $report)
                            <li class="border-b pb-2">
                                ...
                                <div class="text-sm text-gray-500">
                                    {{ $report->created_at->format('Y-m-d H:i') }} - {{ $report->user->name ?? '不明' }}
                                    @if ($report->course)
