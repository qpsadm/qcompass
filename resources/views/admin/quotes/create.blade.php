    ...
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form action="{{ isset($quote) ? route('admin.quotes.update', $quote->id) : route('admin.quotes.store') }}"
                ...
                <input type="text" name="quote_full" class="w-full border px-2 py-1"
                    value="{{ old('quote_full', $quote->quote_full ?? '') }}" required>

                {{-- 原作者名 --}}
                <label class="block mt-2">原作者名</label>
                <input type="text" name="author_full" class="w-full border px-2 py-1"
                ...
                @foreach (['A', 'B', 'C'] as $part)
                    ...
                    <input type="text" name="quote_parts[{{ $part }}]" class="w-full border px-2 py-1"
                        value="{{ old('quote_parts.' . $part, isset($quote) ? optional($quote->quoteParts->where('part_type', $part)->first())->text : '') }}"
                        required>
                @endforeach
                ...
                @foreach (['A', 'B', 'C'] as $part)
                    ...
                    @endphp
                    <input type="text" name="author_parts[{{ $part }}]" class="w-full border px-2 py-1"
                        value="{{ $authorValue }}" required>
                @endforeach

                <button type="submit"
                    class="bg-green-500 text-white px-4 py-2 rounded mt-4">{{ isset($quote) ? '更新' : '登録' }}</button>
            </form>
        </div>
    @endsection
