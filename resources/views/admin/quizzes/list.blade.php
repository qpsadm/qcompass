...
@section('content')
    <div class="container mx-auto p-6 max-w-2xl">
        ...
        <div class="bg-white shadow-lg rounded-xl p-6 border border-gray-200">
            ...
            <form
                action="{{ $question->id ? route('admin.questions.update', $question->id) : route('admin.questions.store') }}"
                method="POST">
                @csrf
                @if ($question->id)
                    @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="question_type" class="border px-2 py-1 w-full rounded" required>
                        <option value="single"
                            {{ old('question_type', $question->question_type ?? '') == 'single' ? 'selected' : '' }}>単一選択
                        </option>
                        <option value="multiple"
                            {{ old('question_type', $question->question_type ?? '') == 'multiple' ? 'selected' : '' }}>
                            複数選択</option>
                        <option value="text"
                            {{ old('question_type', $question->question_type ?? '') == 'text' ? 'selected' : '' }}>記述式
                        </option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="score" value="{{ old('score', $question->score ?? 0) }}"
                                ...
                                <div class="flex gap-2 mb-2">
                                    ...
                                    <label class="flex items-center gap-1">
                                        <input type="checkbox" name="choices[{{ $i }}][is_correct]"
                                            value="1"
                                            {{ !empty($choice['is_correct'] ?? $choice->is_correct) ? 'checked' : '' }}>
                                        正解
                                    </label>
                                </div>
                            @endforeach
