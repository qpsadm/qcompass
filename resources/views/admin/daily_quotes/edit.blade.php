...
<div class="container mx-auto p-6 pb-24 max-w-lg">
    ...
    <form action="{{ route('admin.daily_quotes.update', $DailyQuote->id) }}" method="POST"
        ...
        <!-- quote -->
        <div>
            <label for="quote" class="block text-gray-700 font-semibold mb-2">quote</label>
            <input type="text" name="quote" id="quote"
                value="{{ old('quote', $DailyQuote->quote) }}"
        ...
        <div>
            ...
            <input type="text" name="author" id="author"
                value="{{ old('author', $DailyQuote->author) }}"
                class="w-full border-gray-300 rounded-md px-3 py-2 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>

        <!-- display_date（date に統一） -->
        <div>
            <label for="display_date" class="block text-gray-700 font-semibold mb-2">display_date</label>
        ...
        <!-- is_show（select に統一） -->
        <div>
            <label for="is_show" class="block text-gray-700 font-semibold mb-2">is_show</label>
