...
<div class="container mx-auto p-4 min-h-screen bg-white rounded-lg shadow-md"
    ...
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold text-gray-800">質疑応答一覧</h1>
        ...
        <table class="table-auto border-collapse border w-full text-sm">
            <thead class="bg-gray-100 text-gray-700">
                <tr>
                    ...
                    <th class="border px-3 py-2">講座</th>
                    <th class="border px-3 py-2">質問タイトル</th>
                    <th class="border px-3 py-2">回答講師</th>
                    <th class="border px-3 py-2 text-center">公開/非公開</th>
                    <th class="border px-3 py-2 w-64 text-center">操作</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="border px-3 py-2">{{ $question->course->course_name ?? '-' }}</td>
                    <td class="border px-3 py-2">{{ $question->title }}</td>
                    <td class="border px-3 py-2">{{ $question->responder->name ?? '-' }}</td>
                    <td class="border px-3 py-2 text-center">
                        @if ($question->is_show)
                    ...
                    <td class="border px-3 py-2 text-center">
                        <div class="flex justify-center space-x-2">
                            <a href="{{ route('admin.questions.show', $question->id) }}"
                                class="flex items-center px-2 py-1 text-green-600 hover:text-green-700">
                                <img src="{{ asset('assets/images/icon/b_agenda.svg') }}" class="w-4 h-4 mr-1">
                                <span class="hidden lg:inline">詳細</span>
                            </a>
                            <a href="{{ route('admin.questions.edit', $question->id) }}"
                                class="flex items-center px-2 py-1 text-blue-600 hover:text-blue-700">
                                <img src="{{ asset('assets/images/icon/b_report.svg') }}" class="w-4 h-4 mr-1">
                                <span class="hidden lg:inline">編集</span>
                            </a>
                            <button @click="open = true; deleteUrl='{{ route('admin.questions.destroy', $question->id) }}'; deleteTitle='{{ $question->title }}';"
                                class="flex items-center px-2 py-1 text-red-600 hover:text-red-700">
