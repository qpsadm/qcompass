        ...
        <div x-show="tab === 'basic'">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST"
                ...
                <div>
                    ...
                    <input type="number" name="courses_id" value="{{ old('courses_id', $user->courses_id) }}"
                        class="border px-3 py-2 rounded w-full">
                </div>
        ...
        <div x-show="tab === 'detail'" x-cloak>
            ...
            <form
                ...
                <div>
                    ...
                    <input type="text" name="theme_color" value="{{ old('theme_color', $d->theme_color ?? '') }}"
                        class="border px-3 py-2 rounded w-full">
                </div>

                <div>
                    <label class="block font-semibold mb-1">自己紹介</label>
                    <textarea name="bio" rows="4" class="border px-3 py-2 rounded w-full">{{ old('bio', $d->bio ?? '') }}</textarea>
                </div>

                <div>
                    <label class="block font-semibold mb-1">メモ</label>
                    <textarea name="memo1" rows="4" class="border px-3 py-2 rounded w-full">{{ old('memo1', $d->memo1 ?? '') }}</textarea>
                </div>

                <div>
                    <label class="block font-semibold mb-1">備考</label>
                    <textarea name="memo2" rows="4" class="border px-3 py-2 rounded w-full">{{ old('memo2', $d->memo2 ?? '') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="leaving_reason" rows="4" class="border px-3 py-2 rounded w-full">{{ old('leaving_reason', $d->leaving_reason ?? '') }}</textarea>
                </div>
