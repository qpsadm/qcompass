...
@section('content')
    <div class="container max-w-5xl">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mx-auto">
            ...
            <h1 class="text-2xl font-bold mb-6 text-gray-800">{{ $course->course_name }} のアジェンダ一覧</h1>

            <a href="{{ url()->previous() }}"
                class="back px-4 py-2 bg-blue-600 text-white rounded hover:bg-gray-300 flex items-center justify-center space-x-1 w-40">
            ...
            @foreach ($course->categories as $category)
                <div class="mt-6 p-4 border rounded bg-gray-100">
                    <div class="font-semibold text-lg pb-1"><span
                            class="text-sm text-gray-500">カテゴリ名：</span>{{ $category->name }}
                        ({{ $category->code }})
                    </div>
                    ...
                    @if ($category->agendas->count())
                        <ul class="mt-2 space-y-2">
                            @foreach ($category->agendas as $agenda)
                                <li
                                    class="flex flex-row justify-between gap-2 items-center px-4 hover:bg-gray-100 transition w-full">
                                    ...
                                    <div class="flex flex-row justify-between items-center text-medium text-gray-600 gap-2">
