...
@section('content')
    <div class="container mx-auto p-4 min-h-screen bg-white rounded-lg shadow-md">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ url()->previous() }}"
                class="px-4 py-3 bg-blue-600 text-white rounded hover:bg-gray-300 flex items-center space-x-1">
        ...
        @foreach ($course->categories as $category)
            <div class="mt-6">
                <h2 class="font-semibold text-lg border-gray-300 pb-1">{{ $category->name }} ({{ $category->code }})
                </h2>
                ...
                @if ($category->agendas->count())
                    <ul class="mt-2 space-y-2">
                        @foreach ($category->agendas as $agenda)
                            <li
                                class="flex flex-row items-center p-3 bg-gray-50 border border-gray-200 rounded hover:bg-gray-100 transition w-full">

                                <a href="{{ route('admin.agendas.preview', $agenda->id) }}" target="_blank"
                                    class="flex flex-row items-center text-blue-600 hover:underline font-medium w-1/2">
                                    {{ $agenda->agenda_name }}
                                </a>
                                ...
                                <div class="flex flex-row justify-between items-center text-medium text-gray-600 gap-6">
                                    ...
                                    <div class="px-2 py-1">
                                        更新：{{ $agenda->updated_at }} / {{ $agenda->updated_user_name }}
