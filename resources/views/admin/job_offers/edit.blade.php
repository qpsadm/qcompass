        ...
        <form
            ...
            <table class="w-full table-auto border-collapse">
                <tbody>
                    {{-- 求人タイトル --}}
                    <tr class="border-b">
                        <th class="w-1/4 px-4 py-2 bg-gray-100 text-right font-medium">求人タイトル <span
                                class="bg-red-500 text-white text-xs px-2 py-0.5 rounded ml-1">必須</span></th>
                        <td class="px-4 py-2">
                    ...
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2">
                            <textarea x-ref="descriptionTextarea" x-model="description" name="description" class="border rounded px-3 py-2 w-full"
                                rows="8">{{ old('description', $job_offer->description ?? '') }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                    ...
                    @foreach (range(1, 5) as $i)
                        ...
                        <tr class="border-b">
                            ...
                            <td class="px-4 py-2 flex flex-col gap-2" x-data="{
                                fileName: '',
                                copied: false,
                                ...
                                get sanitizedFileName() {
                                    ...
                                    {{-- 半角の英字（a-z）」と「半角の数字（0-9）」以外の文字をすべて削除（空文字に置き換え）します --}}
                                ...
                                get virtualUrl() {
                                    ...
                                    return `${this.baseUrl}/${'{{ $virtualfilename }}'}${this.sanitizedFileName}`;
                                },
                                copy(url) {
                                ...
                                <input type="file" id="{{ $inputName }}" name="{{ $inputName }}"
                                    class="border rounded px-3 py-2" @change="fileName = $event.target.files[0]?.name ?? ''"
                                ...
                                <template x-if="fileName">
                                    <div class="flex items-center gap-2 text-sm break-all">
                                        ...
                                        <a :href="virtualUrl" target="_blank" class="text-blue-600 underline break-all"
                                            x-text="virtualUrl"></a>
                                ...
                                @if (isset($job_offer) && $job_offer->$column)
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button type="button"
                                            @click="copy('{{ asset('storage/' . $job_offer->$column) }}')"
                                ...
                                @if (isset($job_offer) && $job_offer->{$column})
                                    ...
                                    <p class="text-sm break-all">
                                        ...
                                        <a href="{{ asset('storage/' . $job_offer->{$column}) }}" target="_blank"
                                            ...
                                            {{ asset('storage/' . $job_offer->{$column}) }}
                                        </a>
                                    </p>
