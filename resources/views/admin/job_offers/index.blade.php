            ...
            <table class="table-auto border-collapse border w-full text-sm">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        {{-- <th class="border px-4 py-2 w-16">
                            <a href="{{ route('admin.job_offers.index', array_merge(request()->query(), ['sort' => 'id', 'order' => $sort === 'id' ? $nextOrder : 'asc'])) }}"
                                class="flex items-center justify-center gap-1 hover:underline">
                                ID @if ($sort === 'id')
                                    <span>{{ $order === 'asc' ? '▲' : '▼' }}</span>
                    ...
                    @forelse ($job_offers as $jobOffer)
                        <tr class="hover:bg-gray-50">
                            <td class="border px-4 py-2 text-center">
                                {{ ($job_offers->currentPage() - 1) * $job_offers->perPage() + $loop->iteration }}</td>
                            ...
                            <td class="border px-4 py-2 text-center">
                                <span
                                    class="px-2 py-1 {{ $jobOffer->is_show ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-700' }} rounded-full text-xs">
