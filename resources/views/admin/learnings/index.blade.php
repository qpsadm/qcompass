...
$direction = ($currentSort === $column && $currentDirection === 'asc')
? 'desc'
: 'asc';
...
return '<a href="' . e($url) . '" class="flex items-center justify-center gap-1 hover:underline">' .
    e($label) . $arrow .
    '</a>';
}
@endphp
...
<div class="container mx-auto p-4 min-h-screen bg-white rounded-lg shadow-md">
    ...
    <div class="flex items-center justify-between mb-4 space-x-2">
        ...
        {{-- 検索・絞り込み --}}
        <form method="GET"
            action="{{ route('admin.learnings.index') }}"
        ...
        <table class="table-auto border-collapse border w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="border px-4 py-2">{{ $types[$learning->type] ?? '-' }}</td>
                    ...
                    <td class="border px-4 py-2">{{ Str::limit($learning->description, 50) }}</td>
                    <td class="border px-4 py-2 text-center">
                        @if ($learning->image)
                    ...
                    <td class="border px-4 py-2 text-center">
                        @if ($learning->url)
                        <a href="{{ $learning->url }}" target="_blank" class="text-blue-600 hover:underline">リンク</a>
                        @else
                        -
                        @endif
                    </td>
                    <td class="border px-4 py-2">{{ $levelLabels[$learning->level] ?? '-' }}</td>
                    <td class="border px-4 py-2">{{ $tagLabels[$learning->tag_id] ?? '-' }}</td>
                    <td class="border px-4 py-2 text-center">
                        <span class="px-2 py-1 {{ $learning->is_show ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-700' }} rounded-full text-xs">
