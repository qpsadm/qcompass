            ...
            <form action="{{ route('admin.reports.update', $Report->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="created_user_name"
                        value="{{ old('created_user_name', $Report->created_user_name ?? '') }}"
                        class="border px-2 py-1 w-full rounded">
                </div>
                <div class="mb-4">
                    <label class="block font-medium mb-1">更新者</label>
                    <input type="text" name="updated_user_name"
                        value="{{ old('updated_user_name', $Report->updated_user_name ?? '') }}"
                        class="border px-2 py-1 w-full rounded">
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="deleted_user_name"
                        value="{{ old('deleted_user_name', $Report->deleted_user_name ?? '') }}"
                        class="border px-2 py-1 w-full rounded">
                </div>

                <button type="submit" class="save bg-green-500 text-white px-4 py-2 rounded">更新</button>
            </form>
        </div>
    @endsection
