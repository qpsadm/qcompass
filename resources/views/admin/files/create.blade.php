...
@section('content')
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form method="POST" action="{{ route('admin.files.store', ['type' => $type, 'targetId' => $target->id ?? 0]) }}"
                        ...
                        <tr>
                            ...
                            <td class="border px-4 py-2">
                                <input type="text" name="file_name"
                                    value="{{ old('file_name', $defaultFileName ?? '') }}" placeholder="アップロードファイルの拡張子が付きます"
                ...
                <div class="mt-6 flex gap-3">
                    <button class="save bg-blue-500 text-white px-6 py-2 rounded">
                        保存
                    </button>

                    @if (!empty($returnUrl))
                        <a href="{{ $returnUrl }}" class="back bg-gray-500 text-white px-6 py-2 rounded">
